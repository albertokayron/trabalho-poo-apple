...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h1 class="text-3xl font-bold tracking-tight text-apple-black mt-2">Editar Funcionário</h1>
        <p class="text-sm text-apple-gray mt-1">Altere as informações cadastrais do funcionário.</p>
    </div>
    ...
    <div class="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
        <form action="{{ route('usuarios.update', $usuario->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $usuario->name) }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-1 focus:ring-apple-black focus:border-apple-black text-sm transition-all" required>
            </div>

            <!-- Campo E-mail -->
            <div>
                <label for="email" class="block text-xs font-bold text-apple-black uppercase tracking-wider mb-2">E-mail Corporativo</label>
                <input type="email" name="email" id="email" value="{{ old('email', $usuario->email) }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-1 focus:ring-apple-black focus:border-apple-black text-sm transition-all" required>
            </div>
            ...
            <div>
                <label for="password" class="block text-xs font-bold text-apple-black uppercase tracking-wider mb-2">Nova Senha (Opcional)</label>
                <input type="password" name="password" id="password" placeholder="Deixe em branco para manter a senha atual" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-1 focus:ring-apple-black focus:border-apple-black text-sm transition-all">
                <p class="text-[10px] text-apple-gray mt-1">Preencha apenas se desejar redefinir a senha do funcionário.</p>
            </div>
