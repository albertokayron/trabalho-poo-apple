...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h1 class="text-3xl font-bold tracking-tight text-apple-black mt-2">Registrar Manutenção</h1>
        <p class="text-sm text-apple-gray mt-1">Abra uma nova ordem de serviço para conserto de dispositivo.</p>
    </div>
        ...
        <form action="{{ route('manutencoes.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                @if($aparelhos->isEmpty())
                    <p class="text-xs text-rose-500 mt-1">Atenção: Cadastre primeiro um aparelho no sistema!</p>
                @endif
            </div>
            ...
            <div>
                ...
                @if($usuarios->isEmpty())
                    <p class="text-xs text-rose-500 mt-1">Atenção: Cadastre primeiro um funcionário no sistema!</p>
                @endif
            </div>
            ...
            <div>
                ...
                <textarea name="descricao_problema" id="descricao_problema" rows="4" placeholder="Descreva os defeitos relatados pelo cliente e testes preliminares..." class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-1 focus:ring-apple-black focus:border-apple-black text-sm transition-all" required>{{ old('descricao_problema') }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="data_entrada" id="data_entrada" value="{{ old('data_entrada', now()->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-1 focus:ring-apple-black focus:border-apple-black text-sm transition-all" required>
            </div>
            ...
            <div class="pt-4 border-t border-gray-100 flex items-center justify-end space-x-4">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-full text-xs font-semibold text-white bg-apple-black hover:bg-black hover:shadow transition-all duration-200" {{ ($aparelhos->isEmpty() || $usuarios->isEmpty()) ? 'disabled' : '' }}>
                    Registrar
                </button>
            </div>
        </form>
